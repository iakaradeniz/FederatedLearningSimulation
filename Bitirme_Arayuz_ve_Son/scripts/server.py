...
def serve(
    ...
    # Ortak kök sertifika (CA), sunucu anahtarı ve sunucu sertifikası
    with open('./server/ca.crt', 'rb') as f:
        root_cert = f.read()
    with open('./server/server.key', 'rb') as f:
        private_key = f.read()
    with open('./server/server.crt', 'rb') as f:
        certificate_chain = f.read()

    # Sunucu kimlik bilgileri (mTLS için özel ayarlar)
    server_credentials = grpc.ssl_server_credentials(
        ((private_key, certificate_chain),),
        root_certificates=root_cert,
        require_client_auth=True # İSTEMCİYİ KESİNLİKLE DOĞRULA
    )

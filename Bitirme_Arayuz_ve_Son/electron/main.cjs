...
const PYTHON_DIR = path.resolve(__dirname, "../scripts");
...
ipcMain.handle("run-inference", async (_event, { imagePath, modelPath, arch }) => {
  return new Promise((resolve) => {
    ...
    const script = PYTHON_DIR + "/inference.py";
    //const script = PYTHON_DIR;
    const args = [
      script,
      "--image",      imagePath,
...
ipcMain.on('terminale-yaz', (event, mesaj) => {
  console.log(">>> [Electron Terminal]:", mesaj);
});
...
ipcMain.handle('select-file', async (event, options) => {
  ...
  if (result.canceled) {
    return null; // Kullanıcı iptal ettiyse boş dön
  } else {
    return result.filePaths[0]; // Seçilen dosyanın yolunu dön
  }
});
